...
baseDirectory <- '/home/nick/Data/PigLung/'
...
dataDirectories <- c()
dataDirectories <- append( dataDirectories, paste0( baseDirectory, "Images_LowResolution/" ) )

lungImageFiles <- c()
for( i in seq_len( length( dataDirectories ) ) )
  {
  imageFiles <- list.files( path = dataDirectories[i],
    pattern = "Pig.*Low.nii.gz", full.names = TRUE, recursive = TRUE )
...
for( i in seq_len( length( lungImageFiles ) ) )
  ...
  lungMaskFile <- paste0( subjectDirectory, "/../Masks_LowResolution/", subjectId, "_m.nii.gz" )
  ...
  if( ! file.exists( lungImageFile ) || ! file.exists( lungMaskFile ) )
    {
    stop( paste( "Image or mask file does not exist.\n" ) )
    }
...
weightsFile <- paste0( scriptsDirectory, "/pigLungExtractionWeights.h5" )
